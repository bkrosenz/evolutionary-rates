...
def sim_tree_covs(mu: float, tree_height: float):
    ...
    for j in range(nsims):
        t = treesim.birth_death_tree(birth_rate=lam, death_rate=mu, max_time=tree_height,ntax=4000)
...
@timeout(TIMEOUT)
def sim_tree_covs(mu: float, tree_height: float, lam: float):
    """returns list of records with child node height (age) and branch length.
...
def sim_bd_trees(
    ...
    parent_trees = Parallel(njobs)(delayed(treesim.birth_death_tree)(
...
def simulate_gene_trees(data_dir: Path,
    ...
    if parent_trees is None:
        parent_trees = [dendropy.Tree.get_from_string(s, schema="newick", rooting="force-rooted") for s in (
            data_dir/'parent_trees_4plus.phy').open('r')]  # TreeList.get(path=) will create a shared namespace
...
def simulate_c_star_matrices(data_dir: Path,
    ...
    n_parent_trees = len(
        (data_dir/'parent_trees_4plus.phy').open('r').readlines())
    try:
        if overwrite:
