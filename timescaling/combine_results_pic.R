# invocation:
# parallel -j2 Rscript combine_results_pic  {} 8  ::: `find /N/project/phyloML/rate_timescaling/data/tree_sims/ -name 'h*' -type d -maxdepth 3` ::: rate_trend EB delta BM
#### parameters
require(tidyverse)
# require(crunch)
require(zeallot)
...
get_feature <- function(results, feature, flatten=T) {
  x = do.call(
    "cbind",
    lapply(
      results,
      function(res) {
        matrix(lapply(
          ...
          function(r) {
            if (length(r) > 1) {
              if (endsWith(feature, "lb")) {
                r[["geig_sig2_ci"]][1]
              } else if (endsWith(feature, "ub")) {
                r[["geig_sig2_ci"]][1]
              } else {
                r[[feature]]
              }
  ...
  if (flatten) return( unlist(t(x))) # must convert null to na
  return (x)
}

get_full_results_pic <- function(globstr, ngenes) {
  if (ngenes == 0) {
  ...
  for (d in Sys.glob(globstr, dirmark = FALSE)) {
    ...
    if (file.exists(filename)) {
      ...
      # a$lb_geig <- get_feature(results, "geig_lb")
      # a$ub_geig <- get_feature(results, "geig_ub")
      #  a$geig_ub <- do.call("cbind", lapply(results, function(res) (matrix(lapply(res, function(r) (r$geig_sig2_ci[2]))))))
      print(length(results))
      a <- data.frame(
        tree_ix = rep(0:(length(results)-1), each=20),
        sig_ape = get_feature(results = results, "ape_sig2"),
        sig_geig = get_feature(results = results, "geig_sig2"),
        se_ape = get_feature(results = results, "ape_sig2_se"),
        se_geig = get_feature(results = results, "geig_sig2_se"),
        lam = lam,
        h = h,
        mu = mu,
        r = lam - mu,
      ...
      print(colnames(a))
      # a = a %>% separate_rows(sig_geig,sig_ape,se_ape,se_geig)
...
predictions <- get_full_results_pic(
...
predictions %>% write_csv("../data/bm_geig_ape_preds.csv.gz")
...
for (m in algnames) {
  ...
  ggplot(
    predictions %>% filter(model == m),
    aes(sig, fill = as.factor(model))
  ) +
    geom_histogram() +
    scale_x_log10()
...
ggplot(
  ...
  # geom_point(size=1) +
  geom_smooth(level = 0.50, method = "gam") +
  scale_y_log10() +
...
pac <- pac %>%
  unnest(accelerating) %>%
  pivot_longer(cols = starts_with("V"), values_to = "accelerating", names_to = "block") %>%
  drop_na()
...
for (m in c("delta", "rate_trend", "EB")) {
  ...
  pm <- ggpairs(
    ...
    rdf %>%
      filter(model == !!m) %>%
      select(c(accelerating, CI, r, rho, h)),
...
f %>%
  filter(CI.ils - CI.null > .7) %>%
  select(accelerating.ils, accelerating.null, CI.ils, CI.null, lam, mu, h, r, rho, r_h, rho_h)
...
cp <- preds %>%
  filter(!is.na(CI)) %>%
  group_by(model, hr) %>%
  summarise(EI_CI = mean(CI))
ip <- preds %>%
  filter(!is.na(accelerating)) %>%
  group_by(model, hr) %>%
  summarise(EI = mean(accelerating))
ggplot(ip) +
  aes(x = hr, y = EI, color = as.factor(model)) +
  geom_point() +
  labs(x = "Hr", y = expression(EI)) +
  theme(legend.position = "none") #+      guides(color = guide_legend(title = "Model"))
ggsave("../figures/timescaling/accel.png")

ggplot(cp) +
  aes(x = hr, y = EI_CI, color = as.factor(model)) +
  geom_point() +
  labs(x = "Hr", y = expression("EI"["CI"])) +
